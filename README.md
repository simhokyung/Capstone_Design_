...
AI 예측과 최적제어 알고리즘을 적용해 **에너지 절감**과 **사전 대응 기능**을 갖춘 가정용 공기질 관리 플랫폼을 개발한다.
...
그러나 실내 환경에서 **적절한 환기, 청정이 이루어지지 않을 경우** 


오염물질이 쌓이게 되며 이는 **인체에 악영향**을 끼칩니다. 
...
**1.AI기반 공기질 예측**: 운영 환경에서의 데이터를 수집하여 학습하고, 현재 상태를 바탕으로 1시간 내의 실내 공기질 상태를 1분 단위로 예측한다. 


**2.예측값 기반 사전 대응**: 예측값을 바탕으로 공기청정기의 최적 제어 스케줄을 10분 단위로 제공한다. 


**3.필터 수명 계산**: 필터 수명은 단순 시간 누적이 아니라, 공기청정기의 팬 속도와 공기질 지수(AQI)를 가중치로 반영해 실제 사용 환경을 고려해 계산한다.


**4.맞춤형 공기질 관리 제공**: 사용자에게 천식, 알러지 여부를 입력받아 공기질 관리 기준치를 설정하여 사용자 맞춤형 공기질 관리 서비스를 제공한다. 


**5.시각화 정보 제공(그래프, 히트맵)**: 다수의 IoT센서를 활용해 센서간 간격을 좁혀 해상도 높은 데이터를 사용, 사용자에게 실내 공기질 상태를 히트맵 형식으로 제공해 오염원을 파악할 수 있게 한다.  
